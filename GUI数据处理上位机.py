...
class DataProcessingApp:
    ...
    def __init__(self, root):
        self.root = root
        self.root.title("老王出品 ： 远程数据处理  6.5 V")
    ...
    def no_process_data(self):
        ...
        try:
            if os.path.exists(adress1):  # 检查文件（文件名，文件路径是对得上）是否存在，不存在则结束程序
                ...
                对于工作表[1]中的单元格：
                    单元格_obj = 单元格
                    cell_obj.alignment = openpyxl.styles.Alignment(wrap_text=True,horizo​​ntal='center',
                                                                   垂直='中心'）

                工作簿.保存（文件路径）

                # 表格页面合并完成后，重新启用“发电数据处理”按钮。条件为假时，执行下面代码
                如果不是 self.process_button_disabled：
                    self.process_button.config（状态=tk.NORMAL）

                # 表格页面合并完成后，重新启用“发电数据处理”按钮。条件为假时，执行下面代码
                如果不是 self.excel_process_button_disabled：
                    self.excel_process_button.config（状态=tk.NORMAL）

                # 完成所有操作后更新细节条至 100%
                对于范围（10）内的 _：
                    self.progress['value'] += 1 # 逐步增加进展条值
                    self.progress.update()
                    time.sleep(0.05) # 微小的延迟，实现平滑更新
                #停止详细条动画
                self.progress.stop()
